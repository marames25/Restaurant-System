...
public class ItemsFactory {
    ...
    private final Map<String, Supplier<FoodItem>> inventory;
    ...
    public FoodItem getItem(String id) {
        Supplier<FoodItem> sup = inventory.get(id);
        return (sup == null) ? null : sup.get();
    }

    public List<FoodItem> getAllItems() {
        List<FoodItem> list = new ArrayList<>();
        for (Supplier<FoodItem> s : inventory.values()) {
            list.add(s.get()); // each item fresh
        }
        return list;
    }
}
